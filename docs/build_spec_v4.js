...
// ───────────────────────────── 10. ライセンス・著作権設計
children.push(h1("10. ライセンス・著作権設計"));
children.push(quote("AI音楽を、店舗で安心して流せる場所。 — これが Sotto. の差別化の核。ライセンス保証はブランドの一部である。"));

children.push(h2("10.1 設計の基本方針"));
children.push(p("Sotto. は単なる「投稿サービス」ではなく、店舗が商用利用する音楽の流通基盤になる。したがって、著作権処理は「ユーザー責任」だけでは不十分で、Sotto. が間に立って一定の保証を提供する構造を持つ。"));
children.push(blank());

children.push(table([2400, 6900], [
  new TableRow({ children: [
    tcell("基本方針", { shade: COLOR_BG_GOLD, bold: true, width: 2400 }),
    tcell("詳細", { shade: COLOR_BG_GOLD, bold: true, width: 6900 }),
  ]}),
  new TableRow({ children: [
    tcell("① クリエイター責任", { shade: COLOR_BG_INK, bold: true, width: 2400 }),
    tcell("投稿時に、AIツールの商用利用ライセンスを所持していることを保証する。", { width: 6900 }),
  ]}),
  new TableRow({ children: [
    tcell("② Sotto. への許諾", { shade: COLOR_BG_INK, bold: true, width: 2400 }),
    tcell("投稿者は Sotto. に対し、表示・配信・サムネ生成・OGP・プロモーション等の非独占的利用を許諾する。", { width: 6900 }),
  ]}),
  new TableRow({ children: [
    tcell("③ Spaces 採用時のライセンス", { shade: COLOR_BG_INK, bold: true, width: 2400 }),
    tcell("店舗が Premium 契約すれば、契約範囲内の商用利用が許諾される。クリエイターには採用ごとにロイヤリティ支払い。", { width: 6900 }),
  ]}),
  new TableRow({ children: [
    tcell("④ Sotto. の保証", { shade: COLOR_BG_INK, bold: true, width: 2400 }),
    tcell("Sotto. は店舗オーナーに対し「採用楽曲の利用範囲内でのクレームに一定範囲で対応する」と約束する（Premium 契約条項）。", { width: 6900 }),
  ]}),
  new TableRow({ children: [
    tcell("⑤ DMCA / 削除フロー", { shade: COLOR_BG_INK, bold: true, width: 2400 }),
    tcell("権利者から削除申請があった場合、24時間以内に当該楽曲を非公開化、投稿者に通知し、反論期間を設ける。", { width: 6900 }),
  ]}),
]));

children.push(h2("10.2 投稿フォームの保証チェック"));
children.push(p("クリエイターが作品を投稿するとき、以下のチェックを必須とする："));
children.push(blank());
children.push(bullet("□ この楽曲は私が生成・制作したものであり、第三者の権利を侵害しないことを保証します"));
children.push(bullet("□ 使用した AI ツール（Suno / Udio 等）の商用利用ライセンスを保有しています"));
children.push(bullet("□ Sotto. への配信・表示・プロモーション利用を許諾します"));
children.push(bullet("□ Sotto Spaces 候補にする場合：店舗が商用利用することに同意します"));
children.push(blank());
children.push(p("各チェックは独立し、最後3つは任意（ただし Spaces 候補にするには3つ目以降のチェックが必要）。", { color: COLOR_MUTE, italics: true }));

children.push(h2("10.3 投稿時に記録するメタデータ"));
children.push(p("将来の紛争時のために、投稿時に以下を works テーブルに記録する："));
children.push(blank());
children.push(table([3000, 6300], [
  new TableRow({ children: [
    tcell("ai_tools", { shade: COLOR_BG_INK, bold: true, width: 3000 }),
    tcell("使用したAIツール名（複数可）", { width: 6300 }),
  ]}),
  new TableRow({ children: [
    tcell("prompt", { shade: COLOR_BG_INK, bold: true, width: 3000 }),
    tcell("生成プロンプト（保証の根拠 / 監査用）", { width: 6300 }),
  ]}),
  new TableRow({ children: [
    tcell("license_confirmed_at", { shade: COLOR_BG_INK, bold: true, width: 3000 }),
    tcell("商用ライセンス保証チェックを行った日時", { width: 6300 }),
  ]}),
  new TableRow({ children: [
    tcell("license_confirmed_ip", { shade: COLOR_BG_INK, bold: true, width: 3000 }),
    tcell("チェック時の IP アドレス（証拠保全）", { width: 6300 }),
  ]}),
  new TableRow({ children: [
    tcell("tos_version", { shade: COLOR_BG_INK, bold: true, width: 3000 }),
    tcell("同意した利用規約のバージョン番号", { width: 6300 }),
  ]}),
]));

children.push(h2("10.4 店舗側（Sotto Spaces）への保証構造"));
children.push(p("店舗オーナーが「これ商用利用しても大丈夫？」と聞かれた時、Sotto. が答えられる構造を作る："));
children.push(blank());
children.push(quote("Sotto. Premium 契約中の店舗様は、Sotto Spaces 経由で再生される楽曲について、契約期間中の商用利用を許諾されています。万一第三者から権利クレームがあった場合、Sotto. が一次対応窓口となり、楽曲の即時差替え＋月額返金で対応します。"));
children.push(blank());
children.push(p("この一文があるだけで、店舗オーナーの不安が解消され、Sotto. の差別化が確立する。", { color: COLOR_MUTE, italics: true }));

children.push(h2("10.5 JASRAC / NexTone との関係"));
children.push(p("日本の店舗BGM利用は、原則として JASRAC / NexTone への支払い義務がある（管理楽曲を流す場合）。Sotto. の楽曲は基本的に AI 生成物で個人クリエイターが投稿したものなので、現時点では JASRAC 等の管理外。"));
children.push(blank());
children.push(table([3000, 6300], [
  new TableRow({ children: [
    tcell("現状（2026年時点）", { shade: COLOR_BG_INK, bold: true, width: 3000 }),
    tcell("AI 生成楽曲は管理楽曲リストに含まれず、店舗側の追加負担なし", { width: 6300 }),
  ]}),
  new TableRow({ children: [
    tcell("将来リスク", { shade: COLOR_BG_INK, bold: true, width: 3000 }),
    tcell("JASRAC が AI 生成物を管理対象に含める可能性。継続ウォッチ必要", { width: 6300 }),
  ]}),
  new TableRow({ children: [
    tcell("対策", { shade: COLOR_BG_INK, bold: true, width: 3000 }),
    tcell("Sotto. として JASRAC / NexTone と早期に対話を始める。「AI 生成物の包括処理スキーム」を提案する", { width: 6300 }),
  ]}),
]));

children.push(h2("10.6 AI 学習データ訴訟への対応"));
children.push(p("2024年6月、米国 RIAA が Suno / Udio を著作権侵害で提訴。判決次第で AI 音楽の合法性が大きく変わる可能性がある。"));
children.push(blank());
children.push(h3("Sotto. の立場"));
children.push(bullet("法務リスクを否定せず、開示する"));
children.push(bullet("利用規約に「AI ツール側の規約変更や法的判断によりサービス内容が変わる可能性」を明記"));
children.push(bullet("訴訟動向を継続ウォッチし、必要時は方針転換できる柔軟性を保つ"));
children.push(bullet("クリエイターには「自分が使うツールの規約は自分で確認すべき」と明示"));

children.push(h2("10.7 文書体系"));
children.push(p("Sotto. が用意すべき法的文書："));
children.push(blank());
children.push(table([2700, 2400, 4200], [
  new TableRow({ children: [
    tcell("文書", { shade: COLOR_BG_GOLD, bold: true, width: 2700 }),
    tcell("対象", { shade: COLOR_BG_GOLD, bold: true, width: 2400 }),
    tcell("公開時期", { shade: COLOR_BG_GOLD, bold: true, width: 4200 }),
  ]}),
  new TableRow({ children: [
    tcell("利用規約（クリエイター向け）", { width: 2700 }),
    tcell("投稿者", { width: 2400 }),
    tcell("Phase 1.x 中（必須）", { width: 4200 }),
  ]}),
  new TableRow({ children: [
    tcell("プライバシーポリシー", { width: 2700 }),
    tcell("全ユーザー", { width: 2400 }),
    tcell("Phase 1.x 中（必須）", { width: 4200 }),
  ]}),
  new TableRow({ children: [
    tcell("特定商取引法表記", { width: 2700 }),
    tcell("有料サービス", { width: 2400 }),
    tcell("Phase 1.5 課金開始前", { width: 4200 }),
  ]}),
  new TableRow({ children: [
    tcell("Sotto Spaces 利用契約", { width: 2700 }),
    tcell("店舗オーナー", { width: 2400 }),
    tcell("Phase 2.3 募集ボード公開時", { width: 4200 }),
  ]}),
  new TableRow({ children: [
    tcell("クリエイターロイヤリティ契約", { width: 2700 }),
    tcell("採用クリエイター", { width: 2400 }),
    tcell("Phase 2.5 報酬分配開始時", { width: 4200 }),
  ]}),
  new TableRow({ children: [
    tcell("DMCA / 削除申請ガイドライン", { width: 2700 }),
    tcell("第三者権利者", { width: 2400 }),
    tcell("Phase 1.x 中", { width: 4200 }),
  ]}),
]));

children.push(h2("10.8 弁護士・専門家の確保"));
children.push(p("Phase 2.5（収益分配）開始前に、以下の専門家との関係を構築する："));
children.push(blank());
children.push(bullet("IT・エンタメ専門の顧問弁護士（1名・月額顧問契約 ¥30,000〜）"));
children.push(bullet("税理士（インボイス対応・国際支払い対応）"));
children.push(bullet("社労士（将来の雇用拡大時）"));
children.push(bullet("保険：プロダクト責任保険・IP 賠償保険"));
children.push(blank());
children.push(p("初期はベンチャー支援に積極的な弁護士事務所のスポット相談（¥10,000-30,000/回）で十分。", { color: COLOR_MUTE, italics: true }));

children.push(h2("10.9 ライセンス機能の実装フェーズ"));
children.push(table([1700, 7600], [
  new TableRow({ children: [
    tcell("Phase 1.x", { shade: COLOR_BG_INK, bold: true, width: 1700 }),
    tcell("投稿時の保証チェック、ToS / Privacy Policy 公開、メタデータ記録", { width: 7600 }),
  ]}),
  new TableRow({ children: [
    tcell("Phase 1.5", { shade: COLOR_BG_INK, bold: true, width: 1700 }),
    tcell("特定商取引法表記、Pro 課金規約", { width: 7600 }),
  ]}),
  new TableRow({ children: [
    tcell("Phase 2.0", { shade: COLOR_BG_INK, bold: true, width: 1700 }),
    tcell("用途タグ宣言＝商用許諾の明示化", { width: 7600 }),
  ]}),
  new TableRow({ children: [
    tcell("Phase 2.3", { shade: COLOR_BG_INK, bold: true, width: 1700 }),
    tcell("Sotto Spaces 利用契約、店舗向けライセンス保証文書", { width: 7600 }),
  ]}),
  new TableRow({ children: [
    tcell("Phase 2.5", { shade: COLOR_BG_INK, bold: true, width: 1700 }),
    tcell("ロイヤリティ契約、Stripe Connect 経由の自動分配契約", { width: 7600 }),
  ]}),
]));
...
// ───────────────────────────── 11. 多言語化（i18n）
children.push(h1("11. 多言語化（i18n）"));
children.push(quote("Sotto. は日本発のブランドだが、AI 音楽は国境を持たない。"));

children.push(h2("11.1 方針"));
children.push(p("Phase 1.6 で日本語・英語の言語切替機能を実装する。理由："));
children.push(bullet("AI 音楽クリエイターはグローバルに分散している（特に Suno / Udio ユーザー）"));
children.push(bullet("Sotto. の世界観（ダーク・ギャラリー / The Sotto Lounge Project）は英語圏でも通用する"));
children.push(bullet("店舗側も、訪日インバウンド向け施設（ホテル・カフェ）は英語版があると安心"));

children.push(h2("11.2 技術選択"));
children.push(table([2400, 6900], [
  new TableRow({ children: [
    tcell("ライブラリ", { shade: COLOR_BG_INK, bold: true, width: 2400 }),
    tcell("next-intl（Next.js 公式推奨、App Router 対応）", { width: 6900 }),
  ]}),
  new TableRow({ children: [
    tcell("URL 構造", { shade: COLOR_BG_INK, bold: true, width: 2400 }),
    tcell("/ja/, /en/ をパスに含む（SEO に有利）", { width: 6900 }),
  ]}),
  new TableRow({ children: [
    tcell("既定言語", { shade: COLOR_BG_INK, bold: true, width: 2400 }),
    tcell("日本語（ブラウザ言語と Accept-Language で自動振り分け）", { width: 6900 }),
  ]}),
  new TableRow({ children: [
    tcell("切替UI", { shade: COLOR_BG_INK, bold: true, width: 2400 }),
    tcell("ヘッダー右上に「JA ⌇ EN」セグメント", { width: 6900 }),
  ]}),
  new TableRow({ children: [
    tcell("メッセージ管理", { shade: COLOR_BG_INK, bold: true, width: 2400 }),
    tcell("messages/ja.json / messages/en.json", { width: 6900 }),
  ]}),
]));

children.push(h2("11.3 翻訳対象の優先順位"));
children.push(number("ナビゲーション / ボタン（最優先）"));
children.push(number("投稿フォームのフィールド名・ヒント"));
children.push(number("エラーメッセージ"));
children.push(number("ランディングコピー（タグライン・コンセプト）"));
children.push(number("Letter ページ（やこさんの想いの英訳）"));
children.push(number("利用規約・プライバシーポリシー（要弁護士チェック）"));

children.push(h2("11.4 ブランド世界観の翻訳ポリシー"));
children.push(p("単純な翻訳ではなく、Sotto. の余白と静けさを英語でも保つ。"));
children.push(blank());
children.push(table([3000, 6300], [
  new TableRow({ children: [
    tcell("日本語", { shade: COLOR_BG_GOLD, bold: true, width: 3000 }),
    tcell("英語", { shade: COLOR_BG_GOLD, bold: true, width: 6300 }),
  ]}),
  new TableRow({ children: [
    tcell("音楽が空間に届く。", { width: 3000 }),
    tcell("Music delivered to spaces.", { width: 6300 }),
  ]}),
  new TableRow({ children: [
    tcell("作品を置く", { width: 3000 }),
    tcell("Place a work", { width: 6300 }),
  ]}),
  new TableRow({ children: [
    tcell("マイギャラリー", { width: 3000 }),
    tcell("My Gallery", { width: 6300 }),
  ]}),
  new TableRow({ children: [
    tcell("1曲を、静かにここに置く。", { width: 3000 }),
    tcell("Place a work, quietly.", { width: 6300 }),
  ]}),
  new TableRow({ children: [
    tcell("余白を残し、誠実に。", { width: 3000 }),
    tcell("Leave space. Stay sincere.", { width: 6300 }),
  ]}),
]));
